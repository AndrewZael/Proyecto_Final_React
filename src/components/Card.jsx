  ...
  return (
    <article className='custom-card text-primary rounded custom-shadow bg-light position-relative px-0'>
        <header className={`text-center ${home ? 'header-home-card d-flex position-relative' : ''}`}>
            <div role="img" className={`photo-user-card rounded-circle shadow-sm bg-secondary border border-soft custom-shadow ${!home ? 'position-absolute start-0 end-0 mx-auto' : 'position-relative'}`} style={{ backgroundImage: `url(${user.profile_picture})` }}></div>
